...
export function check_localStorage() {
    try {
...
//保存options到localStorage
export function save_options_to_localStorage() { // 保存options到localStorage
    try {
        if (values.localStorage_enable) {
...
//创建并加载localStorage中的数据到options中
export function load_options() {
    values.options = (function () { //(函数用于绑定localStorage进行闭包) 使用undefined无法保存到localStorage(但是不影响使用)
