...
export default {
    ...
    recompile_extension_modules: {
        name: "重新编译扩展模块",
        desc: "强制从源代码重新编译扩展模块。允许使用名称模式对要重新编译的扩展模块进行细粒度控制。\n" +
            "当你想确保某些模块从源代码编译以获得更好的优化时很有用。可以多次给出。默认为空。",
    },
    ...
    debugger_choice: {
        name: "调试器选择",
        desc: "选择与 --debugger 选项一起使用的特定调试器。这允许你指定任意调试器，而不是依赖自动检测。\n" +
            "例如：gdb、lldb 或任何自定义调试器。默认为自动选择。",
    },
    ...
    macos_sign_keyring_filename: {
        name: "macOS密钥环文件名",
        desc: "指定用于CI/CD环境中自动代码签名的密钥环文件名。这允许通过直接提供密钥环文件来进行无UI提示的签名。\n" +
            "对自动化构建流程很有用。默认为空。",
    },
    macos_sign_keyring_password: {
        name: "macOS密钥环密码",
        desc: "指定用于自动代码签名的密钥环文件的密码。与 --macos-sign-keyring-filename 配合使用，以在CI/CD工作流中实现非交互式签名。\n" +
            "默认为空。",
    },
    macos_app_version: {
        name: "macOS应用程序版本",
